...
The container image includes a Linux kernel (e.g., in '/usr/lib/modules') for booting. Bootc includes a package manager, allowing you to scrutinise installed and available packages from existing and added repositories. Additionally, with a simple command (bootc usr-overlay), bootc mounts an overlay on /usr, enabling temporary package installation via dnf. After testing, this overlay can be dropped at boot or manually unmounted (mount -l /usr). It is advisable to remove all packages installed this way before unmounting /usr to ensure any configuration files in /etc are also removed.
...
File:
/usr/lib/credstore/home.create.admin

Is named as:
usr__lib__credstore__home.create.admin
...
The aim is to explain basic concepts and share some useful tips to enhance your experience. It is important to note that this template is meant to be modified according to your needs, so it is not necessarily a final product. However, it will work out of the box without any modifications. Although it is tailored to KDE Plasma, most of the concepts and methodologies also apply to other desktop environments.
Explaining the Containerfile step by step
Setup filesystem
If you plan to install software on day 2, after the kde-bootc installation is complete, you may need to link /opt to /var/opt. Otherwise, it will remain an immutable directory that can only be populated by the container build.
...
In this example, I'm including Tailscale, which is commonly used in the community, but the same principle applies to any other source you may add to your repositories. 
...
This is one of the key files to modify to customise the installation to achieve your personal goals.
Remove packages
...
RUN dnf clean all
The criteria used in my list are as follows:
...
- Remove unnecessary packages that are resource-heavy, such as rsyslog, or that bring unnecessary services like dracut-config-rescue.
Configuration
...
Custom configuration for new users' home directories will be added to /etc/skel/. This project adds .bashrc.d/kde-bootc to customise bash and can be tweaked as needed.
...
The GITHUB_TOKEN can be created from global settings (not the repository settings) by selecting Developer settings / Personal access tokens / Tokens (classic). The only required scope for the token is read:packages, which will allow you to download your image. 
...
echo <USERNAME>:<GITHUB_TOKEN> | base64
...
I opted for systemd-homed users because they are better suited than regular users with immutable desktops, preventing potential drift if /etc/passwd is modified locally. Additionally, each user home benefits from LUKS encrypted volume.
The process begins when firstboot-setup runs, triggered by firstboot-setup.service during boot. It executes homectl firstboot, which checks if any regular home areas exist. If none are found, it searches for service credentials starting with "home.create." to create users at boot. Service credentials are imported into the systemd service using the following parameter:
ImportCredential=home.create.*

For more details, refer to the homectl and systemd.exec manual pages.

The homed identity file sets the user's parameters, including username, real name, storage type, etc. This file can be modified to customise your user(s). This example sets up a homed area using LUKS storage and a BTRF filesystem. I noticed that for LUKS storage, at least two parameters must be defined: diskSize and rebalanceWeight. If either is missing, systemd-homed will add a perMachine section defining these parameters, which will take precedence over the top-level user record (you may have set) because perMachine sections supersede them.
...
hashedPassword: This is the hashed version of the password stored under secret. Setting up an initial password allows homectl firstboot to create the user without prompting. This password should be changed afterwards (homectl passwd admin). The hash can be created using the mkpasswd utility. 
...
To verify that the mount has been pre-mapped (LUKS storage), run the findmnt or mount command. The /var/home/admin mount point will show idmapped in the parameters, indicating that only the available ranges are being used. 
The available range is 524288…1879048191. In this example, I chose 1000001 to easily identify the service running in the container. For instance, if the container is running Apache with id=48, the volume or folder bound to it will have id=1000048, and so on.
...
The second service automates updates, replacing the default bootc-fetch-apply-updates service, which comes pre-enabled and downloads and applies updates as soon as they are available. This approach is problematic because it causes your computer to shut down without warning, so it is better to disable bootc-fetch-apply-updates service by masking the timer: 
RUN systemctl mask bootc-fetch-apply-updates.timer
The replacement all-system-update service will download a container image (if available) and queue it for the next reboot. It will also update Flatpak packages and, if present, update Toolbox. It is not enabled by default and can be turned on or off by the user as needed.
...
Creating an ISO to install your kde-bootc desktop is simple. First, create the container image either on GitHub or locally. Follow the instructions below to build the container locally, and ensure you do this as root so bootc-image-builder can use the image to make the ISO.
cd /path-to-your-repo
sudo podman build -t kde-bootc .
Then, create a folder named "output" for the ISO image. Next, set up Anaconda's installer by creating a .toml file.
./config.toml
...
The first step is to restore the SELinux context for the systemd-homed home directory. Without this, you may not be able to log in as admin. 
To complete this task, log in as root, activate the admin’s home area, and then run restorecon to restore the SELinux context. 
homectl activate admin
<< enter password for admin: Temp#Pass
restorecon -R /home/admin
homectl deactivate admin

At this point, you can change the passwords for both root and admin: 
passwd root
homectl passwd admin

After completing these steps, log out from root and log in to admin. 
...
Same as above, ou cannot set up the avatar from Plasma's user settings, but you can copy an available avatar (PNG file) from Plasma's avatar directory to the account service's directory. Ensure it is named the same as the username: 
...
To pull your image from a private GitHub repository using podman, copy /usr/lib/ostree/auth.json to /home/admin/.config/containers/auth.json for user space and /root/.config/containers/auth.json for root.
